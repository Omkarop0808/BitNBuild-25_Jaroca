    ...
    <div className="bg-white rounded-lg shadow-sm p-6">
      ...
      <div className="mb-6">
        ...
        <div className="flex items-center space-x-3 mb-2">
          <span className="text-4xl font-bold text-gray-900">
            {product.overall_rating || '4.2'}
          </span>
          <Star className="w-6 h-6 text-green-500 fill-current" />
          <span className="text-sm text-gray-600">
            based on {product.total_reviews_count || '0'} ratings
          </span>
        </div>
        <p className="text-xs text-gray-500">
          Ratings since {product.ratings_updated || 'N/A'}
        </p>
        <p className="text-sm text-gray-700 mt-1">
          Top product: {product.overall_rating || '4.2'} stars
        </p>
      </div>
      ...
      <div className="space-y-2 mb-8">
        {ratingDistribution.length > 0 ? (
          ...
          <div className="text-sm text-gray-500">No rating distribution data available</div>
        )}
      </div>
    </div>
  );
